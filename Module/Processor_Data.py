...
class DataProcessor:
    ...
    def preprocess_data(self) -> None:
        ...
        # Chuẩn hóa tên các columns: loại bỏ khoảng trắng và chuyển về chữ thường
        self.data_2023.columns        = [col.strip().lower() for col in self.data_2023.columns]
        self.data_2024.columns        = [col.strip().lower() for col in self.data_2024.columns]
        self.data_2025_ct2006.columns = [col.strip().lower() for col in self.data_2025_ct2006.columns]
        self.data_2025_ct2018.columns = [col.strip().lower() for col in self.data_2025_ct2018.columns]
        
        # Chuẩn hóa dữ liệu số: chuyển các cột điểm về kiểu float
